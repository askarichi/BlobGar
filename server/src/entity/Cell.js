...
class Cell {
    ...
    checkBorder(border) {
        ...
        if (border && border.isCircle) {
            let centerX = border.centerX || 0,
                centerY = border.centerY || 0,
                limit = Math.max(0, (border.radius || 0) - size),
                dx = this.position.x - centerX,
                dy = this.position.y - centerY,
                dist = Math.sqrt(dx * dx + dy * dy);
            if (dist > limit && dist > 0) {
                let scale = limit / dist;
                this.position.x = centerX + dx * scale;
                this.position.y = centerY + dy * scale;
            } else if (dist > limit) {
