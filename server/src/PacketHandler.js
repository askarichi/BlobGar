...
class PacketHandler {
    ...
    onCompleted(protocol) {
        ...
        this.socket.sendPacket(new Packet.SetBorder(this.socket.playerTracker, this.gameServer.border, this.gameServer.config.serverGamemode, "NOX Arena " + this.gameServer.version));
        this.gameServer.sendChatMessage(null, this.socket.playerTracker, "NOX Arena " + this.gameServer.version);
    ...
    onKeyE(message) {
        if (message.length !== 1) return;
        this.socket.playerTracker.pressE();
    }
    onKeyR(message) {
        if (message.length !== 1) return;
        this.socket.playerTracker.pressR();
    }
    onKeyT(message) {
    ...
    onKeyS(message) {
        if (message.length !== 1) return;
        this.socket.playerTracker.pressF();
    }
    onKeyJ(message) {
        if (message.length !== 1 || !this.socket.playerTracker.OP.enabled) return;
    ...
    randomSkin() {
        let skin = null,
            skinPool = Array.isArray(this.gameServer.availableSkins) && this.gameServer.availableSkins.length ? this.gameServer.availableSkins : this.randomSkins;
        if (skinPool.length) skin = skinPool[Math.floor(Math.random() * skinPool.length)];
        return skin;
    }
    nickName(text) {
        let name = "",
