...
class PlayerTracker {
    constructor(gameServer, socket) {
        ...
        this.frozen = false;
        this.customSpeed = 0;
        this.profileAbilities = null;
        this.profileAbilityEffects = null;
        this.profileResources = null;
        this.noxAbilityState = {
            shieldUntil: 0,
    ...
    resolveSkin(skin) {
        let value = typeof skin === "string" ? skin.trim() : "",
            available = Array.isArray(this.gameServer.availableSkins) ? this.gameServer.availableSkins : [];
        if (!value) value = "Base";
        if (available.length && available.indexOf(value) === -1) value = available.indexOf("Base") !== -1 ? "Base" : available[0];
        return value || "Base";
    }
    setSkin(skin) {
        skin = this.resolveSkin(skin);
        this._skin = skin;
        let writer = new BinaryWriter();
    ...
    updateScale() {
        ...
        if (totalSize === 0) {
            ...
            let baseScale = Math.pow(Math.min(64 / totalSize, 1), .4),
                splitCompensation = 1;
            if (this.cells.length > 4) splitCompensation += Math.min(.12, (this.cells.length - 4) * .01);
            this._scale = Math.min(1, baseScale * splitCompensation);
        }
        this.massChanged = 0;
    }
    joinGame(name, skin) {
        if (this.cells.length) return;
        let requestedName = (name || "").trim() || "Pilot-07",
            authProfile = this.userAuth ? this.gameServer.playerAuth.toProfileIdentity(this.userAuth, this.userAuth.nameLocked ? this.userAuth.displayName : requestedName) : null,
            safeName = authProfile ? authProfile.name : requestedName,
            isNewProfile = !this.gameServer.profileProgressStore.hasProfile(authProfile || safeName),
            progression = (!this.isBot && !this.isMi && !this.isMinion) ? this.gameServer.resolvePlayerAbilityLoadout(authProfile || safeName) : null;
        if (!this.isBot && !this.isMi && !this.isMinion && this.gameServer.adminStore.isMaintenanceEnabled()) {
            this.gameServer.sendChatMessage(null, this, "NOX is in maintenance mode right now.");
            if (this.socket && typeof this.socket.close === "function") this.socket.close(1000, "Maintenance mode");
            return;
        }
        this.profileAbilities = progression ? progression.abilities : null;
        this.profileAbilityEffects = progression ? progression.effects : this.gameServer.defaultPlayerAbilityEffects();
        this.profileResources = progression ? progression.resources : this.gameServer.defaultPlayerResources();
        if (this.userAuth && !this.userAuth.nameLocked) this.userAuth.displayName = safeName;
        this.setSkin(skin);
        this.setName(safeName);
        if (!this.isBot && !this.isMi && !this.isMinion && this.gameServer.adminStore.isPlayerBanned(this._name)) {
            this.gameServer.sendChatMessage(null, this, "This account is banned from NOX.");
            if (this.socket && typeof this.socket.close === "function") this.socket.close(1000, "Banned");
            return;
        }
        this.isMuted = this.gameServer.adminStore.isPlayerMuted(this._name);
        if (!this.isBot && !this.isMi && !this.isMinion) {
            this.gameServer.profileProgressStore.touchProfile(authProfile || this._name, {
                name: safeName,
                skin: this._skin,
                lastSeen: new Date().toISOString(),
                authProvider: authProfile ? authProfile.authProvider : "guest",
                nameLocked: authProfile ? authProfile.nameLocked : false,
                telegramUserId: authProfile ? authProfile.telegramUserId : null
            });
            this.gameServer.adminStore.notePlayerSeen(this._name, isNewProfile, new Date().toISOString());
        }
    ...
    pressE() {
        if (this.gameServer.running && !this.isSpectating) this.gameServer.activateShield(this);
    }
    pressR() {
        if (this.gameServer.running && !this.isSpectating) this.gameServer.launchFreeze(this);
    }
    pressF() {
        if (this.gameServer.running && !this.isSpectating) this.gameServer.deploySpike(this);
    }
    pressQ() {
    ...
    setCenterPos(x, y) {
        if (this.gameServer.border && this.gameServer.border.isCircle) {
            let dx = x - this.gameServer.border.centerX,
                dy = y - this.gameServer.border.centerY,
                dist = Math.sqrt(dx * dx + dy * dy),
                limit = this.gameServer.border.radius;
            if (dist > limit && dist > 0) {
                let scale = limit / dist;
                x = this.gameServer.border.centerX + dx * scale;
                y = this.gameServer.border.centerY + dy * scale;
            }
        }
