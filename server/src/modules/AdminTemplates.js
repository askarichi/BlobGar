...
@media (max-width:1180px){.nox-admin{grid-template-columns:1fr}.nox-admin__sidebar{border-right:0;border-bottom:1px solid rgba(110,168,255,.1)}.nox-kpi,.nox-card--wide,.nox-card--mid{grid-column:span 12}.nox-player-layout{grid-template-columns:1fr}.nox-flag-grid,.nox-resource-admin-grid{grid-template-columns:1fr}}
</style>
</head>
<body>
<div class="nox-admin">
...
<script>
(function(){
  var state = { session: null, csrf: '', featureFlags: {}, currentPlayer: null };
  var sectionMeta = {
  ...
  async function refreshCurrent(){var active=document.querySelector('.nox-admin__section.is-active').getAttribute('data-section');setMessage('', 'Refreshing '+sectionMeta[active].title+'...');if(active==='overview')await loadOverview();if(active==='liveops')await loadLiveOps();if(active==='players')await searchPlayers();if(active==='analytics')await loadAnalytics();if(active==='settings')await loadSettings();if(active==='logs')await loadLogs();setMessage('success', sectionMeta[active].title+' refreshed.');}
